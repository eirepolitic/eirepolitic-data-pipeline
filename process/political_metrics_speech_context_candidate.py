...
SOURCE_KEYS = {
    ...
    "speech_question_context": "processed/oireachtas_unified/latest/metrics/event/speech_question_context/csv/speech_question_context.csv",
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Build deterministic broader speech context inside one candidate batch.")
    ...
    p.add_argument("--report-path", default="speech_context_candidate_report.json")
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    context = build_speech_context(
        ...
        speech_question_context=frames["speech_question_context"],
    ...
    if not audit.get("ready"):
        raise RuntimeError(f"speech_context audit failed: {audit}")
    ...
    result = publish_dataset_to_candidate(
        ...
        batch_id=batch_id,
        frame=context,
        dataset=get_dataset_contract(contract, "speech_context"),
        contract_version=contract_version,
        source_batch_id=batch_id,
    )
    report = {
