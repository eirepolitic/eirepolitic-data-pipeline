"""Builder for the `silver_bill_stages` table."""
...
def build_silver_bill_stages(
    *,
    ...
    """Fetch `/legislation`, normalize bill-stage history, and write outputs."""
    ...
    rows: list[dict[str, Any]] = []
    raw_stage_count = 0
    bills_with_stages: set[str] = set()
    for item in results:
        if not isinstance(item, Mapping):
            continue
        item_rows = _normalise_stage_rows(item, snapshot_date=snapshot_date)
        raw_stage_count += _raw_stage_count(item)
        if item_rows:
            bills_with_stages.add(str(item_rows[0].get("bill_id") or ""))
        rows.extend(item_rows)

    rows = _dedupe_rows(rows, primary_key="bill_stage_id")
    ...
    manifest = {
        ...
        "raw_stage_rows": raw_stage_count,
        "output_rows": int(len(df)),
        "bills_with_stages": len([bill_id for bill_id in bills_with_stages if bill_id]),
...
def _normalise_stage_rows(item: Mapping[str, Any], *, snapshot_date: str) -> list[dict[str, Any]]:
    ...
    stages = bill.get("stages") if isinstance(bill.get("stages"), list) else []

    rows: list[dict[str, Any]] = []
    for stage_index, stage_wrapper in enumerate(stages):
        if not isinstance(stage_wrapper, Mapping):
            continue
        event = stage_wrapper.get("event") if isinstance(stage_wrapper.get("event"), Mapping) else stage_wrapper
        if not isinstance(event, Mapping):
            continue

        stage_uri = _first_text(event, "uri", "stageUri", "stageURI")
        stage_name = _first_text(event, "showAs", "stageName", "name")
        stage_date = _stage_date(event)
        order_in_bill = _first_text(event, "progressStage", "order", "stageOrder")
        stage_outcome = _first_text(event, "stageOutcome", "outcome")
        house = event.get("house") if isinstance(event.get("house"), Mapping) else {}
        chamber = event.get("chamber") if isinstance(event.get("chamber"), Mapping) else {}
        house_uri = _first_text(house, "uri") or _first_text(chamber, "uri")
        house_name = _first_text(house, "showAs", "name") or _first_text(chamber, "showAs", "name")
        bill_stage_id = stage_uri or f"generated:bill_stage:{stable_hash([bill_id, stage_name, stage_date, order_in_bill, house_uri, stage_index], length=24)}"
...
def _stage_date(event: Mapping[str, Any]) -> str | None:
    ...
    if isinstance(dates, list):
        for entry in dates:
            if isinstance(entry, Mapping):
                ...
                parsed = parse_iso_date(entry)
            if parsed:
                parsed_dates.add(parsed)
    direct = parse_iso_date(event.get("date"))
    if direct:
        parsed_dates.add(direct)
    return sorted(parsed_dates)[0] if parsed_dates else None


def _raw_stage_count(item: Mapping[str, Any]) -> int:
    bill = item.get("bill") if isinstance(item.get("bill"), Mapping) else item
    stages = bill.get("stages") if isinstance(bill.get("stages"), list) else []
    return len(stages)
