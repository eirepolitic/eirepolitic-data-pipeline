"""Builder for the `silver_bill_sponsors` table."""
...
from .normalize import stable_hash, stable_record_hash, utc_now_iso
from .schemas import TableSchema

TABLE_NAME = "silver_bill_sponsors"
...
def build_silver_bill_sponsors(
    *,
    ...
    """Fetch `/legislation`, normalize bill sponsors, and write outputs."""
    ...
    rows: list[dict[str, Any]] = []
    raw_sponsor_count = 0
    bills_with_sponsors: set[str] = set()
    for item in results:
        if not isinstance(item, Mapping):
            continue
        item_rows = _normalise_sponsor_rows(item, snapshot_date=snapshot_date)
        raw_sponsor_count += _raw_sponsor_count(item)
        if item_rows:
            bills_with_sponsors.add(str(item_rows[0].get("bill_id") or ""))
        rows.extend(item_rows)

    rows = _dedupe_rows(rows, primary_key="bill_sponsor_id")
    ...
    manifest = {
        ...
        "raw_sponsor_rows": raw_sponsor_count,
...
def _normalise_sponsor_rows(item: Mapping[str, Any], *, snapshot_date: str) -> list[dict[str, Any]]:
    ...
    rows: list[dict[str, Any]] = []
    for sponsor_index, sponsor_wrapper in enumerate(sponsors):
        if not isinstance(sponsor_wrapper, Mapping):
            continue
        sponsor = sponsor_wrapper.get("sponsor") if isinstance(sponsor_wrapper.get("sponsor"), Mapping) else sponsor_wrapper
        if not isinstance(sponsor, Mapping):
            continue
        by = sponsor.get("by") if isinstance(sponsor.get("by"), Mapping) else {}
        as_role = sponsor.get("as") if isinstance(sponsor.get("as"), Mapping) else {}
        sponsor_uri = _first_text(by, "uri")
        sponsor_name = _first_text(by, "showAs", "name")
        sponsor_role_uri = _first_text(as_role, "uri")
        sponsor_role_name = _first_text(as_role, "showAs", "name")
        is_primary = _bool_text(sponsor.get("isPrimary"))
        sponsor_order = str(sponsor_index + 1)
        bill_sponsor_id = f"bill_sponsor:{stable_hash([bill_id, sponsor_uri, sponsor_name, sponsor_role_uri, sponsor_role_name, is_primary, sponsor_order], length=24)}"
        rows.append(
            {
                ...
                "sponsor_role_uri": sponsor_role_uri,
...
def _raw_sponsor_count(item: Mapping[str, Any]) -> int:
    bill = item.get("bill") if isinstance(item.get("bill"), Mapping) else item
    sponsors = bill.get("sponsors") if isinstance(bill.get("sponsors"), list) else []
    return len(sponsors)
