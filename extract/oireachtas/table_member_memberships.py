"""Builder for the `silver_member_memberships` table."""
...
TABLE_NAME = "silver_member_memberships"
...
def build_silver_member_memberships(
    *,
    ...
    """Fetch `/members`, explode memberships, and write silver_member_memberships outputs."""
    ...
    for item in results:
        for member in _iter_member_records(item):
            rows.extend(_normalise_membership_row(member, membership, snapshot_date=snapshot_date) for membership in _iter_memberships(member, item))

    rows = _dedupe_rows(rows, primary_key="membership_id")
...
def _normalise_membership_row(member: Mapping[str, Any], membership: Mapping[str, Any], *, snapshot_date: str) -> dict[str, Any]:
    member_code = _first_text(member, "memberCode", "code", "id")
    member_uri = _first_text(member, "uri", "memberUri")
    membership_uri = _first_text(membership, "uri", "membershipUri")
    house = _first_mapping(membership, "house", "houseRecord")
    membership_start = _membership_start(membership)
    membership_end = _membership_end(membership)
    ...
    return {
        ...
        "is_current": is_current_range(membership_start, membership_end),
        "source_hash": stable_record_hash(membership),
        "snapshot_date": snapshot_date,
    }
...
def _dq_results(df: pd.DataFrame, schema: TableSchema) -> dict[str, Any]:
    ...
    if row_count == 0 or pk not in df.columns:
        non_null_pk = False
        unique_pk = False
        member_populated = False
        house_populated = False
        start_populated = False
    else:
        ...
        house_populated = bool(df["house_uri"].notna().all() and (df["house_uri"].astype(str).str.strip() != "").all())
        start_populated = bool(df["membership_start"].notna().any() and (df["membership_start"].astype(str).str.strip() != "").any())
    status = "pass" if row_count > 0 and not missing_columns and non_null_pk and unique_pk and member_populated and house_populated and start_populated else "fail"
    ...
    return {
        ...
        "checks": [
            ...
            {"check_name": "house_uri_populated", "status": "pass" if house_populated else "fail"},
            {"check_name": "membership_start_any_populated", "status": "pass" if start_populated else "fail"},
        ],
    }
