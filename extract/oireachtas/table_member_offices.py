"""Builder for the `silver_member_offices` table."""
...
def _iter_offices(membership: Mapping[str, Any]) -> Iterable[Mapping[str, Any]]:
    value = membership.get("offices") or membership.get("office")
    if isinstance(value, list):
        for entry in value:
            if isinstance(entry, Mapping):
                yield _unwrap(entry, "office", "officeDetails")
    elif isinstance(value, Mapping):
        yield _unwrap(value, "office", "officeDetails")
...
def _date_start(record: Mapping[str, Any]) -> str | None:
    ...
    office_date_range = dict(record.get("officeDateRange") or {})
    member_date_range = dict(record.get("memberDateRange") or {})
    return parse_iso_date(date_range.get("start") or office_date_range.get("start") or member_date_range.get("start") or record.get("startDate") or record.get("dateStart"))
...
def _date_end(record: Mapping[str, Any]) -> str | None:
    ...
    office_date_range = dict(record.get("officeDateRange") or {})
    member_date_range = dict(record.get("memberDateRange") or {})
    return parse_iso_date(date_range.get("end") or office_date_range.get("end") or member_date_range.get("end") or record.get("endDate") or record.get("dateEnd"))
...
def _dq_results(df: pd.DataFrame, schema: TableSchema) -> dict[str, Any]:
    ...
    if row_count == 0 or pk not in df.columns:
        ...
        uri_ok = bool(df["office_uri"].notna().all() and (df["office_uri"].astype(str).str.strip() != "").all())
        name_ok = bool(df["office_name"].notna().all() and (df["office_name"].astype(str).str.strip() != "").all())
        start_ok = bool(df["office_start"].notna().any() and (df["office_start"].astype(str).str.strip() != "").any())
    status = "pass" if all([row_count > 0, not missing_columns, non_null_pk, unique_pk, membership_ok, member_ok, uri_ok, name_ok, start_ok]) else "fail"
    return {
        "table": TABLE_NAME,
        "dq_status": status,
        "row_count": row_count,
        ...
        "checks": [
            ...
            {"check_name": "office_uri_populated", "status": "pass" if uri_ok else "fail"},
            {"check_name": "office_name_populated", "status": "pass" if name_ok else "fail"},
            {"check_name": "office_start_any_populated", "status": "pass" if start_ok else "fail"},
        ],
    }
