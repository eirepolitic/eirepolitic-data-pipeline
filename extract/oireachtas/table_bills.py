"""Builder for the `silver_bills` table."""
...
def build_silver_bills(
    *,
    ...
    """Fetch `/legislation`, normalize bill-level metadata, and write outputs."""
    ...
    rows = [_normalise_bill_row(item, snapshot_date=snapshot_date, endpoint=endpoint) for item in results if isinstance(item, Mapping)]
    rows = _dedupe_rows(rows, primary_key="bill_id")
    ...
    manifest = {
        ...
        "bill_type_values": sorted(df["bill_type"].dropna().astype(str).unique().tolist()) if not df.empty else [],
        "status_values": sorted(df["status"].dropna().astype(str).unique().tolist()) if not df.empty else [],
        "origin_house_values": sorted(df["origin_house_name"].dropna().astype(str).unique().tolist()) if not df.empty else [],
        ...
        "raw_result_structure": _compact_structure(first_result, max_depth=5),
...
def _normalise_bill_row(item: Mapping[str, Any], *, snapshot_date: str, endpoint: str) -> dict[str, Any]:
    bill = item.get("bill") if isinstance(item.get("bill"), Mapping) else item
    bill_uri = _first_text(bill, "uri", "billUri")
    bill_id = bill_uri or _first_text(bill, "billId", "id") or f"generated:bill:{stable_hash(bill, length=24)}"
    origin_house = bill.get("originHouse") if isinstance(bill.get("originHouse"), Mapping) else {}
    dates = _collect_dates(bill)
    title = _first_text(bill, "shortTitleEn", "longTitleEn", "shortTitleGa", "longTitleGa", "showAs", "title")
    short_title = _first_text(bill, "shortTitleEn", "shortTitleGa")
    bill_type = _first_text(bill, "billType", "method")
    status = _first_text(bill, "status") or _deep_first_text(bill.get("mostRecentStage"), "stageOutcome") or _deep_first_text(bill.get("mostRecentStage"), "showAs")
    return {
        "bill_id": bill_id,
        "bill_uri": bill_uri,
        "bill_no": _first_text(bill, "billNo", "billNumber"),
        "bill_year": _first_text(bill, "billYear", "year"),
        "title": title,
        "short_title": short_title,
        "origin_house_uri": _first_text(origin_house, "uri") or _first_text(bill, "originHouseURI"),
        "origin_house_name": _first_text(origin_house, "showAs", "name"),
        "bill_type": bill_type,
        "status": status,
        "introduced_date": min(dates) if dates else None,
...
def _collect_dates(value: Any) -> list[str]:
    dates: set[str] = set()
    if isinstance(value, Mapping):
        for key, child in value.items():
            key_l = str(key).lower()
            if key_l in {"date", "datetime", "lastupdated", "created", "updated"} or key_l.endswith("date") or key_l.endswith("datetime"):
                parsed = parse_iso_date(child)
                if parsed:
                    dates.add(parsed)
            dates.update(_collect_dates(child))
...
def _deep_first_text(value: Any, target_key: str) -> str | None:
    if isinstance(value, Mapping):
        direct = _first_text(value, target_key)
        if direct:
            return direct
...
def _dq_results(df: pd.DataFrame, schema: TableSchema) -> dict[str, Any]:
    ...
    if row_count == 0 or pk not in df.columns:
        non_null_pk = unique_pk = number_ok = year_ok = title_ok = origin_ok = type_ok = status_ok = introduced_ok = latest_ok = endpoint_ok = False
    else:
        non_null_pk = bool(df[pk].notna().all() and (df[pk].astype(str).str.strip() != "").all())
        unique_pk = bool(not df[pk].duplicated().any())
        number_ok = bool(df["bill_no"].notna().all() and (df["bill_no"].astype(str).str.strip() != "").all())
        year_ok = bool(df["bill_year"].notna().all() and (df["bill_year"].astype(str).str.strip() != "").all())
        title_ok = bool(df["title"].notna().all() and (df["title"].astype(str).str.strip() != "").all())
        origin_ok = bool(df["origin_house_uri"].notna().all() and (df["origin_house_uri"].astype(str).str.strip() != "").all())
        type_ok = bool(df["bill_type"].notna().all() and (df["bill_type"].astype(str).str.strip() != "").all())
        status_ok = bool(df["status"].notna().all() and (df["status"].astype(str).str.strip() != "").all())
        introduced_ok = bool(df["introduced_date"].notna().all() and (df["introduced_date"].astype(str).str.strip() != "").all())
        latest_ok = bool(df["last_event_date"].notna().all() and (df["last_event_date"].astype(str).str.strip() != "").all())
        endpoint_ok = bool((df["source_endpoint"] == "/legislation").all())
    ...
    return {
        ...
        "checks": [
            ...
            {"check_name": "bill_no_populated", "status": "pass" if number_ok else "fail"},
            {"check_name": "bill_year_populated", "status": "pass" if year_ok else "fail"},
            {"check_name": "title_populated", "status": "pass" if title_ok else "fail"},
            {"check_name": "origin_house_uri_populated", "status": "pass" if origin_ok else "fail"},
            {"check_name": "bill_type_populated", "status": "pass" if type_ok else "fail"},
            {"check_name": "status_populated", "status": "pass" if status_ok else "fail"},
            {"check_name": "introduced_date_populated", "status": "pass" if introduced_ok else "fail"},
            {"check_name": "last_event_date_populated", "status": "pass" if latest_ok else "fail"},
            {"check_name": "source_endpoint_legislation", "status": "pass" if endpoint_ok else "fail"},
        ],
    }


def _compact_structure(value: Any, *, depth: int = 0, max_depth: int = 5) -> Any:
    if depth >= max_depth:
