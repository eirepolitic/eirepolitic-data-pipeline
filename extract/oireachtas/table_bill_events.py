"""Builder for the `silver_bill_events` table."""
...
def build_silver_bill_events(
    *,
    ...
    """Fetch `/legislation`, normalize bill events, and write outputs."""
    ...
    rows: list[dict[str, Any]] = []
    raw_event_count = 0
    bills_with_events: set[str] = set()
    for item in results:
        if not isinstance(item, Mapping):
            continue
        item_rows = _normalise_event_rows(item, snapshot_date=snapshot_date)
        raw_event_count += _raw_event_count(item)
        if item_rows:
            bills_with_events.add(str(item_rows[0].get("bill_id") or ""))
        rows.extend(item_rows)

    rows = _dedupe_rows(rows, primary_key="bill_event_id")
...
def _normalise_event_rows(item: Mapping[str, Any], *, snapshot_date: str) -> list[dict[str, Any]]:
    ...
    events = bill.get("events") if isinstance(bill.get("events"), list) else []

    rows: list[dict[str, Any]] = []
    for event_index, event_wrapper in enumerate(events):
        if not isinstance(event_wrapper, Mapping):
            continue
        event = event_wrapper.get("event") if isinstance(event_wrapper.get("event"), Mapping) else event_wrapper
        if not isinstance(event, Mapping):
            continue
        event_uri = _first_text(event, "uri")
        event_type_uri = _first_text(event, "eventURI", "eventUri", "typeURI", "typeUri")
        event_name = _first_text(event, "showAs", "name", "label")
        event_date = _first_event_date(event)
        ...
        event_order = str(event_index + 1)
        bill_event_id = event_uri or f"generated:bill_event:{stable_hash([bill_id, event_type_uri, event_name, event_date, chamber_uri, event_order], length=24)}"
        rows.append(
            {
                "bill_event_id": bill_event_id,
                "bill_id": bill_id,
                "event_uri": event_uri,
...
def _first_event_date(event: Mapping[str, Any]) -> str | None:
    dates = event.get("dates") if isinstance(event.get("dates"), list) else []
    parsed_dates: list[str] = []
    for raw_date in dates:
        if isinstance(raw_date, Mapping):
            parsed = parse_iso_date(raw_date.get("date"))
        else:
...
def _raw_event_count(item: Mapping[str, Any]) -> int:
    bill = item.get("bill") if isinstance(item.get("bill"), Mapping) else item
    events = bill.get("events") if isinstance(bill.get("events"), list) else []
    return len(events)
...
def _dq_results(df: pd.DataFrame, schema: TableSchema) -> dict[str, Any]:
    ...
    if row_count == 0 or pk not in df.columns:
        ...
        event_uri_ok = bool(df["event_uri"].notna().all() and (df["event_uri"].astype(str).str.strip() != "").all())
    ...
    return {
        ...
        "checks": [
            ...
            {"check_name": "event_uri_populated", "status": "pass" if event_uri_ok else "fail"},
