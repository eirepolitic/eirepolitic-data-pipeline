...
def validate_slide_layout(
    *,
    ...
    if whitespace["top_whitespace_ratio"] > max_top:
        errors.append(
            f"top_whitespace_ratio:{whitespace['top_whitespace_ratio']}>{max_top}"
        )
    if whitespace["bottom_whitespace_ratio"] > max_bottom:
        errors.append(
            f"bottom_whitespace_ratio:{whitespace['bottom_whitespace_ratio']}>{max_bottom}"
        )
    if whitespace["occupied_height_ratio"] < min_occupied:
        errors.append(
            f"occupied_height_ratio:{whitespace['occupied_height_ratio']}<{min_occupied}"
        )
    ...
    for element in template.get("elements", []):
        ...
        if float(metrics.get("vertical_fill_ratio", 0.0)) < min_vertical:
            errors.append(
                f"media_vertical_fill:{metrics['element_id']}:{metrics['vertical_fill_ratio']}<{min_vertical}"
            )
        if float(metrics.get("area_fill_ratio", 0.0)) < min_area:
            errors.append(
                f"media_area_fill:{metrics['element_id']}:{metrics['area_fill_ratio']}<{min_area}"
            )
...
def validate_visual_manifest(
    *,
    ...
    errors: list[str] = []
    vertical = float(visual_manifest.get("plot_vertical_fill_ratio", 0.0))
    area = float(visual_manifest.get("plot_area_ratio", 0.0))
    min_vertical = float(rules.get("min_plot_vertical_fill_ratio", 0.0))
    min_area = float(rules.get("min_plot_area_ratio", 0.0))
    if min_vertical and vertical < min_vertical:
        errors.append(f"plot_vertical_fill_ratio:{vertical}<{min_vertical}")
    if min_area and area < min_area:
        errors.append(f"plot_area_ratio:{area}<{min_area}")
    return {
        "success": not errors,
        "errors": errors,
        "warnings": [],
        "metrics": {
            "plot_vertical_fill_ratio": vertical,
            "plot_area_ratio": area,
        },
        "thresholds": {
            "min_plot_vertical_fill_ratio": min_vertical,
            "min_plot_area_ratio": min_area,
        },
    }
