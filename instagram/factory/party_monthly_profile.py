...
CLASSIFIED_KEY = "processed/oireachtas_unified/latest/csv/enrichment_speech_issue_labels.csv"
...
def _draw_title(image: Image.Image, title_lines: list[str]) -> None:
    ...
    if len(title_lines) == 1:
        draw.text((540, 100), title_lines[0], font=_fit_font(draw, title_lines[0], 820, 56, 40), fill=TEXT, anchor="mm")
    else:
        f = _font(45, True)
        draw.text((540, 76), title_lines[0], font=f, fill=TEXT, anchor="mm")
        draw.text((540, 128), title_lines[1], font=f, fill=TEXT, anchor="mm")
...
def _render_chart(path: Path, party: str, period: MonthlyPeriod, title_lines: list[str], supporting: str, rows: list[dict[str, Any]], value_mode: str) -> None:
    ...
    for idx, row in enumerate(rows):
        ...
        wrapped = textwrap.wrap(str(row["label"]), width=23)[:2] or [str(row["label"])]
        if len(wrapped) == 1:
...
def _render_glossary(path: Path) -> None:
    ...
    draw = ImageDraw.Draw(image)
    term_font, body_font = _font(29, True), _font(23)
    y = 225
...
def build(period_value: str, output_root: Path) -> Path:
    ...
    member_party_by_code: dict[str, str] = {}
    member_party_by_name: dict[str, str] = {}
    party_members: dict[str, set[str]] = defaultdict(set)
    for row in members:
        code = _field(row, ["member_code", "Member Code"])
        name = _field(row, ["Full Name", "Member Name", "Name", "full_name"])
        party = _field(row, ["Party", "party", "Party Name"])
        if not party:
            continue
        member_identity = code or name.casefold()
        if not member_identity:
            continue
        if code:
            member_party_by_code[code] = party
        if name:
            member_party_by_name[name.casefold()] = party
        party_members[party].add(member_identity)
    ...
    for row in speeches:
        ...
        if not d or not (period.start <= d <= period.end):
            continue
        month_rows += 1
        category = _field(row, ["PoliticalIssues", "issue_label", "Issue Category", "issue_category", "Category", "classification"])
        if not category or category.upper() == "NONE":
            continue
        classified_month_rows += 1
        speaker_code = _field(row, ["member_code", "speaker_member_code", "Member Code"])
        speaker_name = _field(row, ["Speaker Name", "speaker_name", "Speaker", "Member Name"])
        party = None
        if speaker_code:
            party = member_party_by_code.get(speaker_code)
            if party:
                matched_by_code += 1
        if not party and speaker_name:
            party = member_party_by_name.get(speaker_name.casefold())
            if party:
                matched_by_name += 1
        if not party:
            unmatched[speaker_name or speaker_code or "<blank>"] += 1
            continue
    ...
    for party in parties:
        ...
        for slide_no, p in enumerate(paths, start=1):
            ...
            qa_rows.append({
                ...
                "party_key": key,
                "slide": slide_no,
                "path": str(p.relative_to(period_root)),
                "dimensions_ok": ok,
                "status": "PASS" if ok else "FAIL",
            })
        cover_paths.append((party, paths[0]))
        raw_paths.append((party, paths[1]))
        share_paths.append((party, paths[2]))
        per_td_paths.append((party, paths[3]))
        carousel_items.append((party, paths))
    ...
    lineage = {
        ...
        "readiness": {
            ...
            "matched_by_member_code": matched_by_code,
            "matched_by_name_fallback": matched_by_name,
