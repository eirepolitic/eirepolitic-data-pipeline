...
def _render_cover(path: Path, party: str, speech_count: int, td_count: int, period: MonthlyPeriod) -> None:
    ...
    words = party.split(); split = max(1, len(words)//2); lines = [party] if len(words) == 1 else [" ".join(words[:split]), " ".join(words[split:])]
    party_font = _font(68 if max(map(len, lines)) <= 14 else 54, True)
    if len(lines) == 1: draw.text((cx, cy), lines[0], font=party_font, fill=TEXT, anchor="mm")
    else:
        draw.text((cx, cy-46), lines[0], font=party_font, fill=TEXT, anchor="mm"); draw.text((cx, cy+46), lines[1], font=party_font, fill=TEXT, anchor="mm")
    number_font, label_font, small_font = _font(72, True), _font(25, True), _font(24); avg = speech_count/td_count if td_count else 0.0
    draw.text((294, 955), f"{speech_count:,}", font=number_font, fill=TEXT, anchor="mm"); draw.text((294, 1022), "CLASSIFIED SPEECHES", font=label_font, fill=ACCENT, anchor="mm")
    draw.text((786, 955), f"{avg:.1f}", font=number_font, fill=TEXT, anchor="mm"); draw.text((786, 1022), "AVG SPEECHES PER TD", font=label_font, fill=ACCENT, anchor="mm")
    draw.line((239, 1115, 841, 1115), fill=ACCENT, width=3); draw.text((540, 1170), period.label.upper(), font=label_font, fill=ACCENT, anchor="mm"); draw.text((540, 1218), _period_dates(period), font=small_font, fill=TEXT, anchor="mm")
...
def _render_chart(path: Path, party: str, period: MonthlyPeriod, title_lines: list[str], supporting: str, rows: list[dict[str, Any]], value_mode: str) -> None:
    ...
    draw.text((86, CHART_MEDIA_Y+18), f"{party.upper()} · {period.label.upper()}", font=meta_font, fill=ACCENT, anchor="la"); draw.text((86, CHART_MEDIA_Y+57), supporting, font=support_font, fill=TEXT, anchor="la")
    chart_top, chart_bottom = CHART_MEDIA_Y+105, 1152; row_h = (chart_bottom-chart_top)/max(1, len(rows)); max_value = max((float(r["value"]) for r in rows), default=1.0) or 1.0
    label_x, bar_x, bar_max, value_x = 86, 465, 345, 950
    for idx, row in enumerate(rows):
        cy = chart_top + row_h*(idx+0.5); wrapped = textwrap.wrap(str(row["label"]), width=23)[:2] or [str(row["label"])]
        ...
        value = float(row["value"]); width = max(4, int(bar_max*value/max_value)); draw.rounded_rectangle((bar_x, int(cy-14), bar_x+width, int(cy+14)), radius=8, fill=ACCENT)
    ...
    draw.text((540, 1240), "Dáil speeches · Houses of the Oireachtas / Eirepolitic classification", font=source_font, fill=MUTED, anchor="mm"); path.parent.mkdir(parents=True, exist_ok=True); image.save(path)


def _render_glossary(path: Path) -> None:
    image = _base_slide(); _draw_title(image, ["Glossary"]); draw = ImageDraw.Draw(image); term_font, body_font = _font(29, True), _font(23); y = 225
    for term, body in GLOSSARY:
        draw.text((135, y), term, font=term_font, fill=TEXT, anchor="la"); bbox = draw.textbbox((135, y), term, font=term_font, anchor="la"); underline_y = bbox[3]+8; draw.line((bbox[0], underline_y, bbox[2], underline_y), fill=ACCENT, width=2)
        body_y = underline_y+22
        for line in textwrap.wrap(body, width=79): draw.text((135, body_y), line, font=body_font, fill=TEXT, anchor="la"); body_y += 34
        y = body_y+42
    path.parent.mkdir(parents=True, exist_ok=True); image.save(path)


def _contact_sheet(paths: list[tuple[str, Path]], out_path: Path, columns: int = 4) -> None:
    thumb_w, thumb_h, label_h, gap = 250, 312, 34, 18; rows = math.ceil(len(paths)/columns); canvas = Image.new("RGB", (columns*(thumb_w+gap)+gap, rows*(thumb_h+label_h+gap)+gap), BG); draw = ImageDraw.Draw(canvas); label_font = _font(18, True)
    for idx, (label, path) in enumerate(paths):
        r, c = divmod(idx, columns); x = gap+c*(thumb_w+gap); y = gap+r*(thumb_h+label_h+gap); im = Image.open(path).convert("RGB"); im.thumbnail((thumb_w, thumb_h), Image.Resampling.LANCZOS); canvas.paste(im, (x+(thumb_w-im.width)//2, y)); draw.text((x+thumb_w//2, y+thumb_h+20), label, font=label_font, fill=TEXT, anchor="mm")
...
def _carousel_sheet(items: list[tuple[str, list[Path]]], out_path: Path) -> None:
    thumb_w, thumb_h, row_h, left = 162, 203, 225, 210; canvas = Image.new("RGB", (left+5*thumb_w+35, 30+len(items)*row_h), BG); draw = ImageDraw.Draw(canvas); label_font = _font(20, True)
    for row_idx, (party, paths) in enumerate(items):
        ...
        for col_idx, path in enumerate(paths): im = Image.open(path).convert("RGB"); im.thumbnail((thumb_w, thumb_h), Image.Resampling.LANCZOS); canvas.paste(im, (left+col_idx*thumb_w, y))
    out_path.parent.mkdir(parents=True, exist_ok=True); canvas.save(out_path, quality=92)


def build(period_value: str, output_root: Path) -> Path:
    period = resolve_monthly_period(period_value); labels = _load_labels(); s3 = boto3.client("s3"); speeches, speech_source = _s3_csv(s3, CLASSIFIED_KEY); members, member_source = _member_snapshot_for_period(s3, period)
    ...
    for row in members:
        ...
        if name and party: key = name.casefold(); member_party[key] = party; party_members[party].add(key)
    ...
    for party in parties:
        key = slugify(party); td_count = len(party_members[party]); total = int(party_totals[party]); raw_rows = [{"canonical_label": cat, "label": labels.get(cat, cat), "value": int(count)} for cat, count in party_category[party].most_common(7)]; share_rows=[]; per_td_rows=[]
        ...
        party_manifest={"party":party,"party_key":key,"period":period.key,"td_count":td_count,"classified_speeches":total,"avg_speeches_per_td":round(total/td_count,4) if td_count else None,"raw_counts":raw_rows,"share_vs_average":share_rows,"per_td_vs_average":per_td_rows,"slides":[str(p.relative_to(period_root)) for p in paths],"review_state":"pending_human_review","publication_enabled":False}; manifest_path=parties_root/key/"manifest.json"; manifest_path.parent.mkdir(parents=True,exist_ok=True); manifest_path.write_text(json.dumps(party_manifest,indent=2,ensure_ascii=False),encoding="utf-8"); party_manifests.append(party_manifest)
