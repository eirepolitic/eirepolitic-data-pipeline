...
MUTED = "#c8bda8"
TITLE_RULE_Y = 174
CHART_MEDIA_Y = 184  # 6px gap from the 4px title rule; half the previous 12px gap.

GLOSSARY = [
    (
        "Issues",
        "Dáil speech segments are grouped into political issue categories based on what each segment is mainly about.",
    ),
    (
        "Classified Speeches",
        "Speech segments assigned to one of the issue categories. Counts show how often an issue was discussed, not a party’s position on it.",
    ),
    (
        "Average Party",
        "We calculate the result for each party and then take the simple average across parties, so larger parties do not count more heavily.",
    ),
    (
        "Per TD",
        "The number of classified speeches is divided by the number of TDs in that party. This adjusts the comparison for party size.",
    ),
    (
        "Points vs Average",
        "Shows how far above the average party a result is. For example, +6.1 pts means that issue made up 6.1 percentage points more of a party’s speeches than average.",
    ),
]
...
def _member_snapshot_for_period(s3: Any, period: MonthlyPeriod) -> tuple[list[dict[str, str]], dict[str, Any]]:
    """Use the snapshot immediately after month-end; fail rather than use a later roster."""
    earliest = period.end
    ...
    if not candidates:
        raise RuntimeError(
            f"No period-appropriate member snapshot found for {period.key}. "
            f"Expected {MEMBER_KEY} dated between {earliest} and {latest}; current object is {head_modified}."
        )
...
def _render_chart(
    ...
    draw = ImageDraw.Draw(image)
    meta_font = _font(22, True)
    support_font = _font(24)
    ...
    for idx, row in enumerate(rows):
        cy = chart_top + row_h * (idx + 0.5)
        label = str(row["label"])
        wrapped = textwrap.wrap(label, width=23)[:2] or [label]
        if len(wrapped) == 1:
            draw.text((label_x, cy), wrapped[0], font=label_font, fill=TEXT, anchor="lm")
        else:
            draw.text((label_x, cy - 17), wrapped[0], font=label_font, fill=TEXT, anchor="lm")
            draw.text((label_x, cy + 17), wrapped[1], font=label_font, fill=TEXT, anchor="lm")

        value = float(row["value"])
        width = max(4, int(bar_max * value / max_value))
        draw.rounded_rectangle((bar_x, int(cy - 14), bar_x + width, int(cy + 14)), radius=8, fill=ACCENT)
        if value_mode == "count":
            value_text = f"{int(round(value)):,} speeches"
        elif value_mode == "share_pp":
            value_text = f"+{value:.1f} pts vs avg"
        else:
            value_text = f"+{value:.2f} per TD vs avg"
        draw.text((value_x, cy), value_text, font=value_font, fill=TEXT, anchor="rm")

    draw.text(
        (540, 1240),
        "Dáil speeches · Houses of the Oireachtas / Eirepolitic classification",
        font=source_font,
        fill=MUTED,
        anchor="mm",
    )
...
def _render_glossary(path: Path) -> None:
    image = _base_slide()
    _draw_title(image, ["Glossary"])
    draw = ImageDraw.Draw(image)
    term_font = _font(29, True)
    body_font = _font(23)
    y = 225
    for term, body in GLOSSARY:
        ...
        lines = textwrap.wrap(body, width=79)
...
def _carousel_sheet(items: list[tuple[str, list[Path]]], out_path: Path) -> None:
    thumb_w, thumb_h, row_h = 162, 203, 225
    left = 210
    ...
    for row_idx, (party, paths) in enumerate(items):
        y = 20 + row_idx * row_h
        draw.text((20, y + thumb_h//2), party, font=label_font, fill=TEXT, anchor="lm")
        for col_idx, path in enumerate(paths):
            im = Image.open(path).convert("RGB")
            im.thumbnail((thumb_w, thumb_h), Image.Resampling.LANCZOS)
            canvas.paste(im, (left + col_idx * thumb_w, y))
    out_path.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(out_path, quality=92)


def build(period_value: str, output_root: Path) -> Path:
    period = resolve_monthly_period(period_value)
    labels = _load_labels()
    s3 = boto3.client("s3")
    speeches, speech_source = _s3_csv(s3, CLASSIFIED_KEY)
    members, member_source = _member_snapshot_for_period(s3, period)

    coverage_dates = [d for row in speeches if (d := _parse_date(_field(row, ["Debate Date", "debate_date", "Date"])))]
    if not coverage_dates:
        raise RuntimeError("Classified speech source contains no parseable debate dates")
    coverage_end = max(coverage_dates)
    if coverage_end < period.end:
        raise RuntimeError(f"Classifier coverage ends {coverage_end}; requested period ends {period.end}")
    ...
    for row in members:
        ...
        if name and party:
            key = name.casefold()
            member_party[key] = party
            party_members[party].add(key)

    if not party_members:
        raise RuntimeError("Member snapshot produced no party memberships")

    party_category: dict[str, Counter[str]] = defaultdict(Counter)
    party_totals: Counter[str] = Counter()
    month_rows = 0
    classified_month_rows = 0
    unmatched: Counter[str] = Counter()
    categories: set[str] = set()

    for row in speeches:
        d = _parse_date(_field(row, ["Debate Date", "debate_date", "Date"]))
        if not d or not (period.start <= d <= period.end):
            continue
        month_rows += 1
        ...
        if not category or category.upper() == "NONE":
            continue
        classified_month_rows += 1
        speaker = _field(row, ["Speaker Name", "Speaker", "speaker_name", "Member Name"])
        party = member_party.get(speaker.casefold()) if speaker else None
        if not party:
            unmatched[speaker or "<blank>"] += 1
            continue
        categories.add(category)
        party_category[party][category] += 1
        party_totals[party] += 1

    if month_rows == 0 or classified_month_rows == 0:
        raise RuntimeError(f"No completed classified data found for {period.key}")
    if unmatched:
        preview = ", ".join(f"{k} ({v})" for k, v in unmatched.most_common(10))
        raise RuntimeError(f"Member join is not publication-ready: {sum(unmatched.values())} unmatched classified rows: {preview}")

    parties = sorted(party_members)
    ...
    for category in categories_sorted:
        ...
        for party in parties:
            ...
            td_count = len(party_members[party])
            shares.append(party_category[party][category] / total if total else 0.0)
            rates.append(party_category[party][category] / td_count if td_count else 0.0)
        share_baseline[category] = sum(shares) / len(shares)
        per_td_baseline[category] = sum(rates) / len(rates)

    period_root = output_root / f"period={period.key}"
    parties_root = period_root / "parties"
    ...
    for party in parties:
        key = slugify(party)
        td_count = len(party_members[party])
        total = int(party_totals[party])
        raw_rows = [
            {"canonical_label": cat, "label": labels.get(cat, cat), "value": int(count)}
            for cat, count in party_category[party].most_common(7)
        ]
        share_rows = []
        per_td_rows = []
        for cat in categories_sorted:
            count = int(party_category[party][cat])
            actual_share = count / total if total else 0.0
            share_delta = (actual_share - share_baseline[cat]) * 100.0
            if share_delta > 0:
                share_rows.append({"canonical_label": cat, "label": labels.get(cat, cat), "value": share_delta, "raw_count": count})
            actual_rate = count / td_count if td_count else 0.0
            rate_delta = actual_rate - per_td_baseline[cat]
            if rate_delta > 0:
                per_td_rows.append({"canonical_label": cat, "label": labels.get(cat, cat), "value": rate_delta, "raw_count": count})
        share_rows = sorted(share_rows, key=lambda r: r["value"], reverse=True)[:7]
        per_td_rows = sorted(per_td_rows, key=lambda r: r["value"], reverse=True)[:7]
        if not raw_rows or not share_rows or not per_td_rows:
            raise RuntimeError(f"{party} does not have data for all three analytical slides")

        slides_dir = parties_root / key / "slides"
        paths = [slides_dir / f"0{i}_{name}.png" for i, name in enumerate(
            ["cover", "most_discussed_issues", "more_than_average", "more_per_td", "glossary"], start=1
        )]
        _render_cover(paths[0], party, total, td_count, period)
        _render_chart(paths[1], party, period, ["Most Discussed Issues"], "Total classified speeches", raw_rows, "count")
        _render_chart(paths[2], party, period, ["Issues Discussed", "More Than Average"], "Compared with the average party", share_rows, "share_pp")
        _render_chart(paths[3], party, period, ["Issues Discussed", "More Per TD"], "Adjusted for party size", per_td_rows, "per_td")
        _render_glossary(paths[4])
        ...
        party_manifest = {
            ...
            "avg_speeches_per_td": round(total / td_count, 4) if td_count else None,
            "raw_counts": raw_rows,
            "share_vs_average": share_rows,
            "per_td_vs_average": per_td_rows,
            "slides": [str(p.relative_to(period_root)) for p in paths],
            "review_state": "pending_human_review",
            "publication_enabled": False,
        }
        manifest_path = parties_root / key / "manifest.json"
        manifest_path.parent.mkdir(parents=True, exist_ok=True)
        manifest_path.write_text(json.dumps(party_manifest, indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    contact = period_root / "contact_sheets"
    _contact_sheet(cover_paths, contact / "covers.jpg")
    _contact_sheet(raw_paths, contact / "most_discussed_issues.jpg")
    _contact_sheet(share_paths, contact / "more_than_average.jpg")
    _contact_sheet(per_td_paths, contact / "more_per_td.jpg")
    _carousel_sheet(carousel_items, contact / "five_slide_overview.jpg")
    ...
    lineage = {
        ...
        "period": {"key": period.key, "start": period.start.isoformat(), "end": period.end.isoformat()},
        "readiness": {
            "month_complete": period.end < date.today(),
            "classifier_coverage_end": coverage_end.isoformat(),
            "classifier_covers_period_end": coverage_end >= period.end,
            "month_source_rows": month_rows,
            "classified_month_rows": classified_month_rows,
            "matched_classified_rows": sum(party_totals.values()),
            "unmatched_classified_rows": sum(unmatched.values()),
            "party_count": len(parties),
            "category_count": len(categories_sorted),
        },
        "sources": {"classified_speeches": speech_source, "member_snapshot": member_source},
        "calculation": {
            "raw_counts": "category classified speeches for party in period; top 7",
            "share_vs_average": "party category share minus unweighted mean category share across all parties, including zero shares; positive top 7",
            "per_td_vs_average": "party category speeches per TD minus unweighted mean party category rate per TD across all parties, including zero rates; positive top 7",
        },
        "presentation_labels": str(PRESENTATION_LABELS_PATH),
        "chart_geometry": {"title_rule_y": TITLE_RULE_Y, "chart_media_y": CHART_MEDIA_Y, "previous_main_media_y": 190},
    ...
    print(json.dumps({"status": "PASS", "period": period.key, "parties": len(parties), "slides": len(qa_rows), "output": str(period_root)}, indent=2))
    return period_root


def main() -> None:
    parser = argparse.ArgumentParser(description="Render the reusable monthly party issue profile carousel batch")
    parser.add_argument("--period", default="last_completed_month", help="YYYY-MM or last_completed_month")
    parser.add_argument("--output-root", default=f"instagram/commissioning/output/{PROJECT_ID}")
    args = parser.parse_args()
    build(args.period, Path(args.output_root))
