...
def render(
    ...
    visual_id = str(sample.get("visual_id") or template.get("template_id") or "lollipop_chart_draft_v1")
    params = template.get("params", {}) or {}
    width = int(params.get("width", 1080))
    height = int(params.get("height", 860))
    min_radius = float(params.get("min_radius", 11))
    max_radius = float(params.get("max_radius", 20))
    ...
    if clean_rows:
        ...
        plot_x1 = panel_box[2] - 116
        plot_w = max(1, plot_x1 - plot_x0)
        baseline_ratio = (0.0 - min_value) / span
        baseline_x = plot_x0 + baseline_ratio * plot_w
        baseline_x = max(plot_x0, min(plot_x1, baseline_x))
        top = panel_box[1] + 42
        ...
        draw.line((baseline_x, top - 24, baseline_x, bottom + 24), fill=palette["muted"], width=2)
        ...
        for index, item in enumerate(clean_rows):
            ...
            x = plot_x0 + ratio * plot_w
            radius = min_radius + max(0.0, ratio) * (max_radius - min_radius)
            ...
            draw.line((baseline_x, y, x, y), fill=color, width=5)
            draw.ellipse((x - radius, y - radius, x + radius, y + radius), fill=color, outline=palette["text"], width=2)
            draw.ellipse((x - 4, y - 4, x + 4, y + 4), fill=palette["background"])
            if show_values:
                value_text = _format_value(value, value_format)
                text_x = x + radius + 10 if x >= baseline_x else x - radius - 10 - draw.textbbox((0, 0), value_text, font=value_font)[2]
                draw.text((text_x, y - 10), value_text, font=value_font, fill=palette["text"])
    else:
