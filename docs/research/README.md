...
- [Written PQ semantic classification](parliamentary_questions_semantic_classification.md) — research-only LLM routing investigation covering separate question/answer semantics, deterministic combined topics, controlled hierarchical tags, V2 25/300-section benchmarks, lower-token routing comparison, evidence validation, scale implications and the current human-gold-set next-step plan.
- [Written PQ semantic gold-review set](parliamentary_questions_semantic_gold_set.md) — blind 100-section / 139-question human-review set with grouped, referral, nonstandard-reply, table and answer-length edge cases, plus the annotation procedure and current evaluation plan for comparing rich V2 and lower-token routing against independent human labels.
...
- [Close-division research](close_divisions/README.md) — repeatable research framework for unusually close recorded Dáil divisions, exact proposition reconstruction, debate/external-source analysis, party-at-vote attribution guardrails, first three case files and the living editorial workflow.
- [Close-division Instagram post briefs](close_divisions/post_briefs.md) — production-ready editorial briefs for the Planning for Constitutional Change Bill and counselling-record disclosure amendment, including slide structures, evidence, framing guardrails and source requirements.
