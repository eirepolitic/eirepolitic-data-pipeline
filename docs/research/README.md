...
- [Broader deterministic speech context](broader_speech_context.md) — refreshed deterministic context certification using the live Bill-section bridge; six mutually exclusive non-`other` contexts now cover about 80.8% of speeches, with exact source rules, precedence, rejected approaches, production implications and the living plan for an additive `speech_context` foundation.
