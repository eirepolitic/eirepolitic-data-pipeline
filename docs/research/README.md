...
- [Contextual voting analysis](contextual_voting.md) — read-only investigation linking all current divisions to certified Bill, motion, procedural-business or `other` section context, preserving existing voting denominators, rejecting unsafe stage/text inference, and proposing a future one-row-per-division context foundation.
