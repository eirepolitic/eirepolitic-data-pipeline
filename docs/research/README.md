...
- [Certified Bill-section bridge implementation plan](legislation_bridge_implementation_plan.md) — proposed additive production foundation, exact certified grain, duplicate-source handling, audits, downstream compatibility, deployment sequence, and the current living next step before any architecture change.
- [Irish political data sources](irish_political_data_sources.md) — research-only investigation of Irish polling and other political datasets, including access, licensing, pricing, methodology, automation feasibility, rejected sources, rankings, and future ingestion candidates.
